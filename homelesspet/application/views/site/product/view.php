	...
	<script src="<?php echo public_url('js'); ?>/jqzoom_ev/js/jquery.jqzoom-core.js" type="text/javascript"></script>
		...
		<div class="panel panel-info ">
		  <div class="panel-heading" style="background-image: url(upload/BackGround/tim.jpg)">
		    <h3 style="margin-top:20px;color: white;font-weight:bold;margin-left:39%" class="panel-title">Xem chi tiết sản phẩm</h3>
		  </div>
		  <div class="panel-body" style="background-image:url(upload/BackGround/bg3.jpg);">
                ...
                <div class="text-center">
					...
					<ul id="thumblist" style="margin-top: 20px;">
	                  ...
	                  <?php foreach ($image_list as $value) {?>
	                    <li>
	                      <a style="display: inline;" href='javascript:void(0);' rel="{gallery: 'gal1', smallimage: '<?php echo base_url(); ?>/upload/product/<?php echo $value ?>',largeimage: '<?php echo base_url(); ?>/upload/product/<?php echo $value ?>'}">
	                      <img style="display: inline;" src='<?php echo base_url(); ?>/upload/product/<?php echo $value; ?>'>
	                      </a>
	                  </li>
	                  <?php } ?> <?php endif; ?>
	              </ul>
		  	...
		  	<div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
		  		<h1 style="font-size: 25px;color: #333333;font-weight:strong;"><?php echo $product->name; ?></h1>
		  		<p><?php echo $product->content; ?></p>
		  		<?php 
		  			if ($product->discount > 0) {
		  				$price_new = $product->price - $product->discount;
		  				?><p style="display: inline;">Giá gốc:</p><p style="color: red;"><strong><del><?php echo number_format($product->price) ?> VNĐ</del></strong></p>
						  <p>Giảm giá: <strong><?php echo number_format($product->discount) ?> VNĐ</strong></p>
		  				<p>Giá khuyến mại: <span style="font-weight: bold;color: green"><?php echo number_format($price_new); ?> VNĐ</span></p>
		  			<?php }else{ ?>
		  				<p>Giá: <span style="font-weight: bold;color:blueviolet"><?php echo number_format($product->price); ?> VNĐ</span></p> <?php
		  			}
		  		?>
				<p>Số lượt xem: <?php echo $product->view; ?></p>
		  		...
		  		<a style="background-color: rgb(144, 129, 241);" href="<?php echo base_url('cart/add/'.$product->id); ?>" class="btn btn-info"> Thêm vào giỏ hàng</a>
		  	</div>
		  </div>
		</div>
		<div class="panel panel-info">
		  <div class="panel-heading" style="background-image: url(upload/BackGround/tim.jpg)">
		    <h3 style="margin-top:20px;color: white;font-weight:bold;margin-left:43%" class="panel-title">Sản phẩm liên quan</h3>
		  </div>
		  <div class="panel-body"style="background-image:url(upload/BackGround/bg3.jpg);">
		  	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 clearpadding">
	  			...
	  			?>
					<div class="col-xs-12 col-sm-6 col-md-3 col-lg-3 re-padding">
			  			<div class="product_item" style="background-color: rgba(255, 255, 255, 60%)">
			  				...
			  				<?php	} ?>
							<a href="<?php echo base_url('cart/add/'.$value->id); ?>"><button style="background-color: rgb(144, 129, 241)" class='btn btn-info'> Thêm giỏ hàng</button></a>
			  			</div>
					</div>
				<?php } ?>	
		  	</div>
